...
struct Day23_24_WhyStructsView: View {
    var body: some View {
        ScrollView {
            Text("""
            ...
            In UIKit, every view descended from a class called **`\(Text("UIView").coloredText(.red))`** that had many properties and methods – a background color, constraints that determined how it was positioned, a layer for rendering its contents into, and more. There were lots of these, and every **`\(Text("UIView").coloredText(.red))`** and **`\(Text("UIView").coloredText(.red))`** subclass had to have them, because that’s how inheritance works.
            ...
            You can see this in action when you look at the kinds of things that can be a view. We already used **`\(Text("Color.red").coloredText(.red))`** and **`\(Text("LinearGradient").coloredText(.red))`** as views – trivial types that hold very little data. In fact, you can’t get a great deal simpler than using **`\(Text("Color.red").coloredText(.red))`** as a view: it holds no information other than “fill my space with red”.

            In comparison, **`\(Text("Apple’s documentation for UIView").coloredText(.red))`** lists about 200 properties and methods that **`\(Text("UIView").coloredText(.red))`** has, all of which get passed on to its subclasses whether they need them or not.
